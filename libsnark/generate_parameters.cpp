    ...
    char *params_path, char *input_path)
{
  ...
  {
    // keypair.pk.A_query[i].to_affine_coordinates();
    write_g1<ppT>(params, keypair.pk.A_query[i]);
  }

  for (size_t i = 0; i <= m; ++i)
  {
    // keypair.pk.B_query[i].h.to_affine_coordinates();
    write_g1<ppT>(params, keypair.pk.B_query[i].h);
  }

  for (size_t i = 0; i <= m; ++i)
  {
    // keypair.pk.B_query[i].g.to_affine_coordinates();
    write_g2<ppT>(params, keypair.pk.B_query[i].g);
  }

  for (size_t i = 0; i < m - 1; ++i)
  {
    // keypair.pk.L_query[i].to_affine_coordinates();
    write_g1<ppT>(params, keypair.pk.L_query[i]);
  }

  for (size_t i = 0; i < d; ++i)
  {
    // keypair.pk.H_query[i].to_affine_coordinates();
...
int main(int argc, const char *argv[])
{
  ...
  if (argc > 1)
  {
    ...
    {
      log2_d_4753 = atoi(argv[1]);
      log2_d_6753 = atoi(argv[2]);
    }
  }
  if (log2_d_4753 != 0)
